...
Process16SSequences <- function(filename = "Output/16S_2024/16STree.png", width = 4000, height = 3000) {
  # Start PNG device
  png(filename, width = width, height = height)
  
  # Read the tree from a pre-specified file
  tree <- read.tree("Output/16S_2024/colonized_bacteria.tre")
...
Process16SSequencesSpeciesLabels <- function(filename = "Output/16S_2024/16STreeSpeciesLabels.png", width = 4000, height = 3000, taxonomyFile = "Data/16S_2024/Kangas7753-Taxonomy.xls") {
  # Start PNG device
  png(filename, width = width, height = height)
  ...
  # Read taxonomy data from Excel
  taxonomy <- read_excel(taxonomyFile)
  
  # Ensure the taxonomy data has columns 'Sample Name', 'Genus', and 'species'
  if (!("Sample Name" %in% names(taxonomy)) || !("Genus" %in% names(taxonomy)) || !("species" %in% names(taxonomy))) {
    stop("Taxonomy file must contain 'Sample Name', 'Genus', and 'species' columns")
  }
  
  # Create full species names by combining 'Genus' and 'species'
  taxonomy$FullSpecies <- paste(taxonomy$Genus, taxonomy$species)
  
  # Create a named vector to map sample names to full species names
  species_labels <- setNames(taxonomy$FullSpecies, taxonomy$`Sample Name`)
  
  # Update tree tip labels
  tree$tip.label <- species_labels[tree$tip.label]
  
  # Check for any NA values which indicate missing matches
  if (any(is.na(tree$tip.label))) {
    warning("Some sample names in the tree could not be matched with full species names.")
  }
  
  # Plot the tree with updated species labels
  plot(tree, edge.color = "black", show.node.label = FALSE, show.tip.label = TRUE, cex=3)
...
Process16SSequencesSpeciesLabels()
